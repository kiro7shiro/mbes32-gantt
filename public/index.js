...
class App {
    constructor(eventsData, eventsTodos) {
        ...
        this.ganttChart = EventsGanttChart.buildSync(eventsData, { container: '#gantt-chart', options: this.ganttChartOptions })
        this.eventTodos = EventTodos.buildSync(eventsTodos, { container: '#eventTodos', events: ['click', 'dblclick'] })
        // ui events
        const self = this
        this.ganttChart.on('eventBarClick', function (event) {
            const { detail: id } = event
            const eventData = self.ganttChart.eventsData.find(function (data) {
                return data.id === id
            })
    ...
    // TODO : implement a download functionality for eventData
}
...
async function main() {
    ...
    // parse data
    // TODO : use xlsx-fuzzyparser to import eventData
    const eventsData = EventData.fromArray(eventsDataJson, { blacklist: eventDataBlacklist })
    const eventsTodos = EventTodo.fromArray(eventsTodosJson)
    // load app
    const app = new App(eventsData, eventsTodos)
    window.app = app
}
